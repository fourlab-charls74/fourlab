...
function openProduct(prd_no) {
    ...
    var product = window.open(
        url,
        "_blank",
        "toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=1024,height=900"
    );
}
...
function openStock(goods_no, goods_opt) {
    ...
    var stock = window.open(
        url,
        "_blank",
        "toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=800,height=768"
    );
}
...
function openHeadStock(goods_no, goods_opt) {
    ...
    var stock = window.open(
        url,
        "_blank",
        "toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=1024,height=768"
    );
}

function openDaySales(sales_no) {
    var url = "head/sales/sal02/" + sales_no;
    var sales = window.open(
        url,
        "_blank",
        "toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=1024,height=900"
    );
...
function openUserSelect() {
    ...
    window.open(
        url,
        "_blank",
        "toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=800,height=900"
    );
}
...
function StyleOrdNo(params) {
    if (params.value !== undefined) {
        ...
        if (
            params.data.ord_no_bg_color !== undefined ||
            params.data.ord_no_bg_color != ""
        ) {
            return {
...
function getLength(str){
	...
	for(var i = 0; i < str.length; i++)
	{
		if(encodeURI(str.charAt(i)).length >= 4)
			length += 2;
		else if(encodeURI(str.charAt(i)) == "%A7")
			length += 2;
		else
