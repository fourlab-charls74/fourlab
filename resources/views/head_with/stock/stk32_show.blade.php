									...
									<table class="table incont table-bordered" width="100%" cellspacing="0">
										...
										<tbody>
											<tr>
                                                ...
                                                <td>
													<div class="flax_box">
														<select name="store_cd" class="form-control form-control-sm">
															<option value="">==선택==</option>
															<option value="bonsa">물류창고</option>
															@foreach ($store_info as $list)
																<option value="{{ $list['store_cd'] }}">{{ $list['store_nm'] }}</option>
															@endforeach
														</select>
													</div>
                                                </td>
...
<script type="text/javascript" charset="utf-8">
	...
	function Save() 
	{
		...
		if( $('select[name="store_cd"]').val() === "" )
		{
			alert('등록할 매장은 반드시 입력해야 합니다.');
			$('select[name="store_cd"]').focus();
		...
		if( ret )
		{
			...
			$.ajax({
				async: true,
				type: 'put',
				url: '/head/stock/stk32/show',
				data: {
					data : JSON.stringify(GridData),
					store_cd : $('select[name="store_cd"]').val()
				},
	...
	function Upload(){
		...
		$.ajax({
			url: '/head/stock/stk32/upload', // point to server-side PHP script
