		...
		<div class="card-title">
			<div class="filter_wrap">
				...
				<div class="fr_box flax_box">
					<a href="#" onclick="DeleteData()" class="btn-sm btn btn-primary shadow-sm pl-2 mr-1">상품매칭 삭제</a>
...
<script language="javascript">
	var columns = [
		...
		{field: "sale_stat_cl_val", headerName: "상품상태", width:80, type:'GoodsStateType'},
		{field: "goods_nm", headerName: "상품명", width: 200, type:'HeadGoodsNameType'},
        {field: "goods_nm_eng", headerName: "상품명(영문)", width: 200, type:'HeadGoodsNameType'},
		{field: "goods_opt", headerName: "옵션", width: 120, editable: true, cellStyle:{"background-color":"#FFFF99"}, onCellValueChanged:checkData},
	...
	function ChangeData()
	{
        ...
        {
            alert("선택한 데이터 중 XMD코드와 매칭되어 있지 않은 데이터가 있습니다. XMD코드와 매칭 후에 변경할 수 있습니다.");
			return;
        }
        else
        {
    ...
    function DeleteData()
	{
        ...
        if( not_matched_cnt != 0)
        {
            alert("선택한 데이터 중 XMD코드와 매칭되어 있지 않은 데이터가 있습니다. 매칭되어 있지 않은 데이터는 삭제하실 수 없습니다.");
			return;
        }
        else
        {
            if(confirm("확인 버튼을 누르면 상품매칭 데이터가 삭제됩니다.\r\n삭제하시겠습니까?")) 
            {
                $.ajax({
                	async: true,
                	type: 'put',
                	url: '/head/stock/stk30/delete',
                	data: {
                		data : JSON.stringify(checkRows),
                	},
                	success: function (data) {
                		if( data.code == "200" ){
                			...
                			alert("이미 존재하지 않는 코드 혹은 옵션 데이터입니다.");
                		}else{
