...
<div class="py-3 px-sm-3">
    <div class="page_tit d-flex justify-content-between">
        ...
        <div>
            <a href="javscript:void(0);" class="btn btn-sm btn-primary shadow-sm mr-1 receipt-btn">거래명세표</a>
            <a href="javscript:void(0);" onclick="window.close()" class="btn btn-sm btn-outline-primary shadow-sm"><i class="fas fa-times fa-sm"></i> 닫기</a>
        </div>
    </div>

    <style>
        .custm_tb1 th { padding: 7px 12px 7px 12px; }
        .custm_tb1 td { padding: 5px 12px 5px 12px; }
        .custm_tb2 th { padding: 7px 12px 7px 12px; }
        .custm_tb2 td { padding: 5px 12px 5px 12px;vertical-align: middle;}
        .custm_tb3 tr { padding: 0px 12px 7px 12px; }
        .checked-goods td:not([rowspan='{{count($ord_lists)}}']) { background: yellow; }
        .goods-list tbody th { border: 1px solid #ddd !important;text-align: right; }
        .goods-list tbody th[rowspan='2'] { text-align: center;vertical-align: middle; }
        .claim-list td[colspan='5'] { font-weight: bold;text-align: center; }
    </style>
                                    ...
                                    <table class="table table-bordered th_border_none custm_tb2">
                                        ...
                                        <tbody>
                                            ...
                                            <tr @if ($ord_list->ord_opt_no == $ord_opt_no) class="checked-goods" @endif >
                                                ...
                                                <td><a href="/store/order/ord01/order/{{$ord_no}}/{{$ord_list->ord_opt_no}}">{{@$ord_list->order_state}}</a></td>
                                                <td>
	                                            ...
	                                            <td style="text-align:right">
		                                            ...
		                                            @if(@$ord_list->sale_kind_amt != 0)
			                                            <span class="mt-1">(판매유형)</span>
			                                            <span class="mb-1">{{number_format(@$ord_list->sale_kind_amt ?? 0)}}</span>
		                                            @endif
		                                            @if(@$ord_list->coupon_amt != 0)
			                                            <span class="mt-1">(쿠폰)</span>
			                                            <span class="mb-1">{{number_format((@$ord_list->coupon_amt ?? 0) * -1)}}</span>
		                                            @endif
		                                            @if((@$ord_list->dc_amt ?? 0) - (@$ord_list->sale_kind_amt ?? 0) != 0)
			                                            <span class="mt-1">(수기할인)</span>
			                                            <span class="mb-1">{{number_format((@$ord_list->dc_amt ?? 0) - (@$ord_list->sale_kind_amt ?? 0))}}</span>
													@endif
		                                            </div>
	                                            </td>
                                        ...
                                        <form name="store_refund">
                                            ...
                                            <div class="card-body">
                                                ...
                                                <div class="text-center mt-3">
                                                    <button class="btn-sm btn btn-primary store-refund-save-btn fs-12 mr-1">환불</button>
                                                    <button class="btn-sm btn btn-secondary fs-12" data-dismiss="modal" aria-label="Close">취소</button>
                                                </div>
                                            </div>
                                        </form>
                                    </div>
...
<script>
    ...
    $('.search-btn').click((e) => {
        location.href = "/store/order/ord01/order/" + $('#ord_no').val();
    });
    ...
    $('.store-refund-btn').click(function(e) {
        e.preventDefault();
    ...
    $('.store-refund-save-btn').click(function(e) {
        ...
        axios({
            url: '/store/order/ord01/order/store_refund',
            method: 'post',
            data: frm.serialize(),
        }).then(function (res) {
            if(res.data.code == 200) {
    ...
    function SaveClaimMsg() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                //location.reload();
				location.href	= `/store/order/ord01/order/${ord_no}/${ord_opt_no}?refund_yn=${refund_yn}`;
            },
            error: function(request, status, error) {
                console.log("error")
            }
