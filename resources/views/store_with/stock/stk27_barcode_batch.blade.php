...
<div class="show_layout py-3 px-sm-3">
    <div class="page_tit d-flex justify-content-between">
        <div class="d-flex">
            <h3 class="d-inline-flex">창고재고조정 바코드등록</h3>
            <div class="d-inline-flex location">
                <span class="home"></span>
	            <span>상품관리</span>
	            <span>/ 창고재고조정</span>
                <span>/ 창고재고조정 바코드등록</span>
            </div>
        </div>
        <div class="d-flex">
                                        ...
                                        <tr>
	                                        ...
	                                        <td colspan="5">
		                                        <div class="flex_box">
			                                        ...
			                                        <a href="/sample/sample_stk26_barcode.xlsx" download="창고재고조정바코드등록_샘플" class="ml-2" style="text-decoration: underline !important;">창고재고조정바코드등록 양식다운로드</a>
		                                        </div>
	                                        </td>
                                        </tr>
                                    </tbody>
        ...
        <div class="card shadow mt-3">
            <div class="card-header d-flex justify-content-between align-items-left align-items-sm-center flex-column flex-sm-row mb-0">
                ...
                <div class="d-flex align-items-center">
	                <div class="flex_box mr-1" style="width:150px;">
    ...
    */
    ...
    const getGood = async (values, firstIndex) => {
        axios({
            ...
            let data = mergeDuplicateValues(res.data.body);

            for(let i = 0; i< data.length; i++) {
                data[i].loss_qty = data[i].storage_wqty - data[i].qty;
	...
	// loss사유 일괄입력
    function setAllLossReason() {
		let reason_cd = $("#loss_reason").val();
		let reason_val = $("#loss_reason option:selected").text();
		gx.gridOptions.api.forEachNode(node => {
			node.setDataValue('loss_reason', reason_cd);
			node.setDataValue('loss_reason_val', reason_cd === '' ? '-' : reason_val);
		});
    }
</script>
@stop
