	...
	<div id="search-area" class="search_cum_form">
		<div class="card mb-3">
			...
			<div class="d-flex card-header justify-content-between">
				<h4>검색</h4>
				<div class="flax_box">
					<a href="javascript:void(0);" onclick="openBatchPopup()" class="btn btn-sm btn-outline-primary shadow-sm pl-2 mr-1"><i class="bx bx-plus fs-16"></i> 일괄등록</a>
...
<script type="text/javascript" charset="utf-8">
					...
					if (!e.data.store_cd) {
						...
						$.ajax({
							...
							success: function (res) {
								...
								e.data.total_store_sale_qty = res.total_store_sale_qty || 0;
								e.api.redrawRows({rowNodes:[e.node]});
								gx2.setFocusedWorkingCell();
                                updatePinnedRow();
								console.log(res);
    ...
    function openStorageStockPopup() {
        ...
        openStoreStock(selected_prd.prd_cd);
    }
    ...
    function RequestRT() {
        let rows = gx3.getSelectedRows();
        if(rows.length < 1) return alert("본사요청할 항목을 선택해주세요.");
        if(!confirm("선택한 항목을 본사요청하시겠습니까?")) return;
	...
	const openBatchPopup = () => {
		const url = '/store/stock/stk21/batch';
