...
@section('content')
										...
										<tr>
											...
											<td colspan="5">
												<div class="flex_box">
													...
													<a href="/sample/sample_stk35.xlsx" download="매장일괄반품_샘플" class="ml-2" style="text-decoration: underline !important;">매장일괄반품 양식다운로드</a>
												</div>
											</td>
										</tr>
										</tbody>
	...
	<script language="javascript">
		...
		let columns = [
			...
			{field: "store_cd", headerName: "보내는 매장코드", width: 100, cellStyle: {"text-align": "center"}, hide:true},
			{field: "store_nm", headerName: "보내는 매장", width: 100, cellStyle: {"text-align": "center"}},
	...
	<script type="text/javascript" charset="utf-8">
		...
		const populateGrid = async (workbook) => {
			...
			let values = { data: rows, storage_cd: document.f1.storage_cd.value};
		...
		const upload = () => {
			...
			axios({
				method: 'post',
				url: '/store/stock/stk35/batch-import',
				data: form_data,
				headers: {
		...
		const getGood = async (values, firstIndex) => {
			axios({
				url: '/store/stock/stk35/batch-getgoods',
				method: 'post',
				data: values,
			}).then(async (res) => {
