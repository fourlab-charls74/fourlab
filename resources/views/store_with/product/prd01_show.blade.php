    ...
    <script type="text/javascript" charset="utf-8">
        ...
        function get_category_by_goods_no(cat_type, goods_no, goods_sub)
		{
            $.ajax({
                async: true,
                type: 'get',
                url: '/store/api/category/get_category_by_goods_no/'+cat_type+"/"+goods_no+"/"+goods_sub,
        ...
        function PopPrdDetail(goods_no, goods_sub){
            window.open("/store/product/prd01/"+goods_no,"Product Detail");
        }
        ...
        $("#img-setting").click(function(){
			...
			@else
				window.open("/store/product/prd08/"+goods_no+"/image","_blank","toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=1024,height=960");
			@endif
        });
        ...
        $(".copy-btn").click(function(e){
          ...
          location.href="/store/product/prd01/"+goods_no+"?type=copy";
        });
        ...
        $(".btn-change-qty").click(function(e) {
          ...
          $.ajax({
              async: true,
              type: 'put',
              url: '/store/product/prd01/update/qty',
        ...
        $('.btn-qty-in').click(function(){
            var url = "/store/product/prd01/" + goods_no + "/in-qty";
            ...
            if (type === 'create') {
                ...
                $('.goods-info-change-btn').click(function(e){
                    ...
                    if( confirm("선택하신 상품정보고시 품목으로 변경하시겠습니까?") ){
                        $.ajax({
                            method: 'put',
                            url: '/store/product/prd01/goods-class-opt-update',
                            data: {
                ...
                $('.goods-info-save-btn').click(function(e){
                    ...
                    selectedRowData.forEach(function(data, idx) {
                        ...
                        $.ajax({
                            async: true,
                            type: 'put',
                            url: `/store/product/prd01/goods-class-update`,
                            data: data,
                            success: function (data) {
                    ...
                    if( confirm("삭제하시겠습니까?") ){
                        ...
                        selectedRowData.forEach(function(data, idx) {
                            $.ajax({
                                async: true,
                                type: 'put',
                                url: `/store/product/prd01/goods-class-delete`,
                                data: data,
                                dataType: "json",
                                success: function (data) {
                                    if (selectRowCount -1 === idx) {
                ...
                function goodsClassSearch() {
                    ...
                    gxc.Request(`/store/product/prd01/${goods_no}/goods-class`, data, -1);
                }
        ...
        $(document).ready(function(){
            ...
            $("#sabang_cont").on("click", function() {
                const url = `/store/product/prd01/${goods_no}/goods-cont`;
...
@endif
    ...
    const searchOptKind = () => { // 좌측 옵션 그리드 초기화
        if (goods_no) {
            const data = `goods_no=${goods_no}&goods_sub=${goods_sub}`;
            gx1.Request(`/store/product/prd01/${goods_no}/get-option-name`, data, -1, (response) => {
    ...
    const initRightOptSection = async (type = "기본", columns = []) => {
        ...
        if (type == "기본") {
            const response = await axios({ url: `/store/product/prd01/${goods_no}/get-basic-opts-matrix`, method: 'get' });
    ...
    const getOptionStock = async (row) => {
        ...
        try {
            const response = await axios({ url: `/store/product/prd01/get-option-stock`, method: 'post', data: { data: row, type: type } });
            const { data, status } = response;
            if (status == 200) {
                last_option_row = row;
        ...
        if (type == "기본") {
            ...
            try {
                ...
                const response = await axios({
                    url: `/store/product/prd01/get-extra-options`, method: 'post',
                    data: { 'option_no': no }
                });
    ...
    */
    ...
    function addOptionKind() {
        ...
        $.ajax({
            async: true,
            type: 'post',
            url: `/store/product/prd01/${goods_no}/option-kind-add`,
            data: {
    ...
    function delOptionKind() {
        ...
        $.ajax({
            async: true,
            type: 'post',
            url: `/store/product/prd01/${goods_no}/option-kind-del`,
            data: {
    ...
    */
    ...
    const optDeleteInGrid = async () => {
        ...
        if (Array.isArray(del_opt_list) && !(del_opt_list.length > 0)) {
            ...
            try {
                const response = await axios({ url: `/store/product/prd01/${goods_no}/delete-basic-options`,
    ...
    const updateBasicOptsData = async (rows) => {
        ...
        const response = await axios({ url: `/store/product/prd01/${goods_no}/update-basic-opts-data`,
    ...
    const updateExtraOptsData = async (rows) => {
        ...
        const response = await axios({ url: `/store/product/prd01/update-extra-opts-data`,
    ...
    const openOptsStock = () => { // 상품관리 - 입고 오픈
        const url = `/store/product/prd01/${goods_no}/stock`;
    ...
    function deletePlanning(d_cat_cd){
        ...
        $.ajax({
            type: 'post',
            url: `/store/product/prd01/${goods_no}/planing-delete`,
            data: data,
            dataType: 'json',
            success: function (data) {
                if(data.code === 200) {
    ...
    function deleteCoupon(coupon_no){
        ...
        $.ajax({
            type: 'post',
            url: `/store/product/prd01/${goods_no}/coupon-delete`,
            data: data,
            dataType: 'json',
            success: function (data) {
                if(data.code === 200) {
    ...
    */
    ...
    $(document).ready(function() {
        ...
        $(".similar-add-btn").on("click", function(e) {
            e.preventDefault();
            const url = '/store/product/prd01/choice';
    ...
    function ChoiceGoodsNo(nos, add_goods) {
        $.ajax({
            async: true,
            type: 'post',
            url: `/store/product/prd01/${goods_no}/similar-goods-add`,
            data: {
                'add_goods': add_goods,
                'goods_sub': goods_sub,
            },
    ...
    function DeleteSimilarGoods() {
        ...
        $.ajax({
            async: true,
            type: 'delete',
            url: `/store/product/prd01/${goods_no}/similar-goods-del`,
            data: {
                'del_goods': del_goods,
                'goods_sub': goods_sub,
            },
    ...
    */
    ...
    function get_add_info() {
        $.ajax({
            type: "get",
            url: '/store/product/prd01/' + goods_no + '/get-addinfo',
            dataType: 'json',
            // data: {},
            success: function (res) {
    ...
    const addRelatedGoods = (data) => { // 관련상품 추가
        ...
        $.ajax({
            type: "post",
            url: '/store/product/prd01/add-related-goods',
            dataType: 'json',
            data: {
                cmd: CMD,
    ...
    function delRelatedGood(row) { // 관련상품 삭제
        ...
        $.ajax({
            type: "post",
            url: '/store/product/prd01/del-related-good',
            dataType: 'json',
            data: {
                cmd: CMD,
                goods_no: document.f1.goods_no.value,
    ...
    function openWongaPopup() {
        const url = "/store/product/prd03/wonga?goods_no=" + document.f1.goods_no.value + "&goods_sub=" + document.f1.goods_sub.value;
