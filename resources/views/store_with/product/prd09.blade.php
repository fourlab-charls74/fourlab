...
@section('title', '품번별 재고조회')
@section('content')
		<div class="page_tit d-flex justify-content-between">
			<div class="d-flex">
				<h3 class="d-inline-flex">품번별 재고조회</h3>
					...
					<div class="table-responsive">
						<div class="table-box-ty2 mobile">
							<table class="table incont table-bordered" width="100%" cellspacing="0">
								...
								<tbody>
								<tr>
									<td rowspan="2" class="img_box brln" id="prd_image">
										@if (@$prd->img !== null)
											<img class="goods_img" src="{{config('shop.image_svr')}}/{{@$prd->img}}" alt="이미지" style="min-width: 80px;max-width:80px; min-height: 80px;max-height:80px;" />
										@else
											<p class="d-flex align-items-center justify-content-center" style="min-width: 80px;max-width:80px; min-height: 80px;max-height:80px;">이미지 없음</p>
										@endif
									</td>
	...
	<script language="javascript">
		...
		let storage_columns = [
			...
			{field: "storage_nm", headerName: "창고명", width: 250, cellStyle: AlignCenter, pinned: "left", rowGroup: true, hide: true},
		...
		let store_columns = [
			{field: "color", headerName: "컬러", width: 250, cellStyle: AlignCenter, pinned: "left", rowGroup: true, hide: true,
	...
	<script>
		...
		$(document).ready(async function() {
			...
			if (gridDiv2 !== null) {
				gx2 = new HDGrid(gridDiv2, store_columns, {
					autoGroupColumnDef: basic_autoGroupColumnDef('컬러', 250),
		...
		async function Search() {
			...
			if($("#prd_cd_p").val() == ''){
				alert('품번/바코드는 반드시 입력해야 합니다.');
			}
			
			resetGrid();
			let params = $("form[name=search]").serialize();
			let { data, status } = await axios({ url: "/store/product/prd09/search?" + params, method: "get" });
			if (status === 200) {
		...
		function setGoodsInfo(prd) {
			let img = "";
			if (prd.img) {
				img = `<img class="goods_img" src="{{config('shop.image_svr')}}/${prd.img}" alt="${prd.goods_nm}" style="min-width: 80px;max-width:80px; min-height: 80px;max-height:80px;" />`;
			} else {
				img = `<p class="d-flex align-items-center justify-content-center" style="min-width: 80px;max-width:80px; min-height: 80px;max-height:80px;">이미지 없음</p>`;
			}
			$("#prd_image").html(img);
		...
		function setColumns(sizes) {
			...
			list.push({
				headerName: "합계",
				children: [
					{field: "qty", headerName: "실재고", type: "currencyType", maxWidth: 68, minWidth: 68, sort: "desc",
						aggFunc: (params) => params.values.reduce((a,c) => a + (c * 1), 0),
					},
					{field: "wqty", headerName: "보유재고", type: "currencyType", maxWidth: 65, minWidth: 65,
						aggFunc: (params) => params.values.reduce((a,c) => a + (c * 1), 0),
					},
				],
			});
