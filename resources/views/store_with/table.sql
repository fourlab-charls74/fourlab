...
CREATE TABLE `store` (
	...
	`store_nm_eng` varchar(100) NOT NULL COMMENT '매장명(영문)',
	...
	`shop_yn` char(1) DEFAULT 'N' COMMENT 'shop채널여부',
	`consignment_yn` char(1) DEFAULT 'N' COMMENT 'consignment채널여부',
	`online_yn` char(1) DEFAULT 'N' COMMENT 'online채널여부',
	`wholesale_yn` char(1) DEFAULT 'N' COMMENT 'wholesale채널여부',
	`retail_store_sales_yn` char(1) DEFAULT 'N' COMMENT 'retail_store_sales채널여부',
	...
	`cash` int(11) DEFAULT NULL COMMENT '현금',
	`warranty` int(11) DEFAULT NULL COMMENT '보증보험',
	`deposit_coll` int(11) DEFAULT NULL COMMENT '부동산담보',
	`etc_coll` int(11) DEFAULT NULL COMMENT '기타담보',
	...
	`ostore_stock_yn` char(1) DEFAULT 'Y' COMMENT '타매장재고조회(본 매장 외)',
	`store_stock_yn` char(1) DEFAULT 'N' COMMENT '타매장재고조회',
...
CREATE TABLE `store_return` (
	...
	`storage_cd` varchar(30) NOT NULL COMMENT '창고코드 - storage : storage_cd',
	`store_cd` varchar(30) NOT NULL COMMENT '매장코드 - store : store_cd',
	`sr_date` char(10) NOT NULL COMMENT '반품요청일',
	`sr_pro_date` char(10) NOT NULL COMMENT '반품처리일',
	`sr_fin_date` char(10) NOT NULL COMMENT '반품완료일',
...
CREATE TABLE `stock_check_product` (
	...
	`loss_price` int(11) COMMENT 'LOSS 금액 (loss_rec_qty x price)',
	`loss_price2` int(11) COMMENT 'LOSS 현재가 금액 (store_qty x price)',
	`loss_tag_price` int(11) COMMENT 'LOSS TAG가 금액 (store_qty x LOSS등록 시 tag가)',
	`loss_reason` varchar(10) DEFAULT NULL COMMENT 'LOSS사유 - code : LOSS_REASON',
	`comment` varchar(255) DEFAULT NULL COMMENT 'LOSS상품별 메모',
	`rt` datetime DEFAULT NULL COMMENT '등록일자',
	`ut` datetime DEFAULT NULL COMMENT '수정일자',
	`admin_id` varchar(30) DEFAULT NULL COMMENT '관리자아이디',
...
CREATE TABLE `sproduct_stock_release` (
	`idx` INT(11) NOT NULL AUTO_INCREMENT COMMENT '일련번호',
	`release_no` VARCHAR(30) COMMENT '출고번호',
	...
	`prc_qty` INT(11) DEFAULT NULL COMMENT '출고수량',
	`store_cd` VARCHAR(30) NOT NULL DEFAULT '0' COMMENT '수령매장코드',
	`storage_cd` VARCHAR(30) NOT NULL DEFAULT '0' COMMENT '창고코드',
	`state` INT(11) NOT NULL DEFAULT '0' COMMENT '상태(요청/접수/출고/입고(매장)/거부:10/20/30/40/-10)',
	`exp_dlv_day` VARCHAR(8) DEFAULT NULL COMMENT '출고예정일자',
