...
<script type="text/javascript">
	...
	function Search(callback = null) {
		...
		gx2.Request("/store/pos/search/goods", data, 1, function (e) {
	...
	async function getUserCouponList(user_id = '') {
		...
		const { data: { body }, status } = await axios({ method: "get", url: "/store/pos/search/member-coupon?user_id=" + user_id });
		if (status === 200) {
	...
	async function checkUserId() {
		...
		let { data: cnt, status } = await axios({ url: '/store/member/mem01/check-id/' + user_id, method: 'get'});
	...
	async function checkUserPhone() {
		...
		let { data, status } = await axios({ url: '/store/pos/check-phone?' + form.serialize(), method: 'get'});
		if(status != 200) return alert("중복확인 중 오류가 발생했습니다.\n다시 시도해주세요.");
	...
	function setReservationOrdType(ord_no, ord_opt_no) {
		...
		axios({
			url: '/store/pos/complete-reservation',
			method: 'post',
	...
	async function searchUserInCouponModal() {
		...
		const { data, status } = await axios({
			method: 'get',
			url: "/store/pos/search/member?search_type=phone&search_keyword=" + keyword
		});
		if (status === 200) {
