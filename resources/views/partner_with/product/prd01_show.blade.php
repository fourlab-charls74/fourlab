							...
							<div class="col-4">
								...
								<div class="card-body pt-2">
									...
									<div class="table-responsive">
										<div id="div-gd-optkind" style="height:250px;" class="ag-theme-balham"></div>
									</div>
								</div>
...
<script type="text/javascript" charset="utf-8">
        ...
        function get_category_by_goods_no(cat_type, goods_no, goods_sub)
		{
            $.ajax({
                async: true,
                type: 'get',
                url: '/partner/api/category/get_category_by_goods_no/'+cat_type+"/"+goods_no+"/"+goods_sub,
        ...
        $(".btn-change-qty").click(function(){
          ...
          $.ajax({
              async: true,
              type: 'put',
              url: '/partner/product/prd01/update/qty',
       ...
       /** 상품정보고시 관련 */
        ...
        if ($("#goods-class-grid").length > 0) {
            ...
            if (type === 'create') {
                ...
                $("#create_to_class").on("change", async function (e) {
                    ...
                    const res = await axios({ 'method': 'get', url: "/partner/product/prd05/column_search?class=" + e.target.value });
                    if (res.status === 200) {
                ...
                function goodsClassSearch() {
                    ...
                    gxc.Request(`/partner/product/prd01/${goods_no}/goods-class`, data, -1);
                }
        ...
        $(document).ready(function(){
            ...
            $("#sabang_cont").on("click", function() {
                const url = `/partner/product/prd01/${goods_no}/goods-cont`;
...
@endif
    ...
    const searchOptKind = () => { // 좌측 옵션 그리드 초기화
        if (goods_no) {
            const data = `goods_no=${goods_no}&goods_sub=${goods_sub}`;
            gx1.Request(`/partner/product/prd01/${goods_no}/get-option-name`, data, -1, (response) => {
    ...
    const initRightOptSection = async (type = "기본", columns = []) => {
        ...
        if (type == "기본") {
            const response = await axios({ url: `/partner/product/prd01/${goods_no}/get-basic-opts-matrix`, method: 'get' });
    ...
    const getOptionStock = async (row) => {
        ...
        try {
            const response = await axios({ url: `/partner/product/prd01/get-option-stock`, method: 'post', data: { data: row, type: type } });
            const { data, status } = response;
            if (status == 200) {
                last_option_row = row;
        ...
        if (type == "기본") {
            ...
            try {
                ...
                const response = await axios({
                    url: `/partner/product/prd01/get-extra-options`, method: 'post',
                    data: { 'option_no': no }
                });
    ...
    */
    ...
    function addOptionKind() {
        ...
        $.ajax({
            async: true,
            type: 'post',
            url: `/partner/product/prd01/${goods_no}/option-kind-add`,
            data: {
    ...
    function delOptionKind() {
        ...
        $.ajax({
            async: true,
            type: 'post',
            url: `/partner/product/prd01/${goods_no}/option-kind-del`,
            data: {
    ...
    */
    ...
    const optDeleteInGrid = async () => {
        ...
        if (Array.isArray(del_opt_list) && !(del_opt_list.length > 0)) {
            ...
            try {
                const response = await axios({ url: `/partner/product/prd01/${goods_no}/delete-basic-options`,
    ...
    const updateExtraOptsData = async (rows) => {
        ...
        const response = await axios({ url: `/partner/product/prd01/update-extra-opts-data`,
    ...
    const openOptsStock = () => { // 상품관리 - 입고 오픈
        const url = `/partner/product/prd01/${goods_no}/stock`;
    ...
    function deletePlanning(d_cat_cd){
        ...
        $.ajax({
            type: 'post',
            url: `/partner/product/prd01/${goods_no}/planing-delete`,
            data: data,
            dataType: 'json',
            success: function (data) {
                if(data.code === 200) {
    ...
    function deleteCoupon(coupon_no){
        ...
        $.ajax({
            type: 'post',
            url: `/partner/product/prd01/${goods_no}/coupon-delete`,
            data: data,
            dataType: 'json',
            success: function (data) {
                if(data.code === 200) {
    ...
    */
    ...
    $(document).ready(function() {
        ...
        $(".similar-add-btn").on("click", function(e) {
            e.preventDefault();
            const url = '/partner/product/prd01/choice';
    ...
    function SearchSimilarGoods() {
        ...
        gx3.Request(`/partner/product/prd01/${goods_no}/get-similar-goods`, data, -1);
    }
    ...
    function ChoiceGoodsNo(nos, add_goods) {
        $.ajax({
            async: true,
            type: 'post',
            url: `/partner/product/prd01/${goods_no}/similar-goods-add`,
            data: {
                'add_goods': add_goods,
                'goods_sub': goods_sub,
            },
    ...
    function DeleteSimilarGoods() {
        ...
        $.ajax({
            async: true,
            type: 'delete',
            url: `/partner/product/prd01/${goods_no}/similar-goods-del`,
            data: {
                'del_goods': del_goods,
                'goods_sub': goods_sub,
            },
    ...
    */
	...
	const setOptionStatus = (param) => {
		...
		$.ajax({
			type: "put",
			url: '/partner/product/prd01/' + param.no + '/update-opt-status',
			dataType: 'json',
			data: data,
			success: function (res) {
				if(res.code === 200) {
