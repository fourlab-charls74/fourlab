...
<script type="text/javascript" charset="utf-8">
	...
	const Save = () => {
		...
		if (GridData.length === 0) {
			...
			const rows = GridData;
			axios({
                url: '/shop/sale/sal01/update', method: 'post', data: { data: rows },
				responseType: 'json'
            }).then((response) => {
				const { data } = response;
	...
	// 					url: '/shop/sale/sal01/update', method: 'post', data: { data: row }
	// 				});
	// 				const { data } = response;
	...
	function Upload(){
		...
		$.ajax({
			url: '/shop/sale/sal01/upload', // point to server-side PHP script
