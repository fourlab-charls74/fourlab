                                                ...
                                                <div class="form-inline group-list">
                                                    @foreach($user_groups as $val)
                                                        <div class="btn-group mr-1 mt-2 group-{{$val->group_no}}">
                                                            <button type="button" class="btn btn-outline-info text-left">{{$val->group_nm}}</button>
                                                        </div>
                                                    @endforeach
                                                </div>
                                                @endif
...
<script>
    ...
    const column6 = [
        ...
        {field: "goods_nm", headerName: "상품명", type:"ShopGoodsNameType", pinned:'left'},
    ...
    const column7 = [
        ...
        {field: "ord_no", headerName: "주문번호", type:"ShopOrderNoType"},
    ...
    function openClaimList(user_id, date) {
        ...
        var url = `/shop/member/mem20/pop?user_id=${user_id}&date=${date}`;
    ...
    function openQAList(user_id, date) {
        ...
        var url = `/shop/member/mem21/pop?user_id=${user_id}&date=${date}`;
        window.open(url, "_blank", "toolbar=no,scrollbars=yes,resizable=yes,status=yes,top=500,left=500,width=1024,height=768");
    }

	function openEstimateShow(no){
		var url = "/shop/member/mem22/"+no;
...
<script>
    ...
    function deleteGroup(no) {
        ...
        $.ajax({
            async: true,
            type: 'delete',
            url: `/shop/member/mem01/user/group/${user_id}`,
    ...
    if ($('.add-btn').length > 0) {
        $('.add-btn').click(async function(e){
            ...
            $.ajax({
                async: true,
                type: 'post',
                url: `/shop/member/mem01/user/`,
                data: data,
                success: function (id) {
                    alert("등록되었습니다.");
                    location.href="/shop/member/mem01/show/edit/"+id;
                },
                error: function(request, status, error) {
                    console.log("error")
                }
        ...
        $('.check-user-btn').click(function(e){
            $.ajax({
                async: true,
                type: 'get',
                url: `/shop/member/mem01/check-id/${$('#user_id').val()}`,
    ...
    if ($('.edit-btn').length > 0) {
        ...
        $('.edit-btn').click(async function(e){
            ...
            $.ajax({
                async: true,
                type: 'put',
                url: `/shop/member/mem01/user/${user_id}`,
                data: data,
                success: function (id) {
                    alert("수정되었습니다.");
                    location.reload();
        ...
        $('.change-pw-btn').click(function(){
            ...
            $.ajax({
                async: true,
                type: 'put',
                url: `/shop/member/mem01/pw/${user_id}`,
                data: { pw },
                success: function (id) {
                    alert("수정되었습니다.");
                    location.reload();
