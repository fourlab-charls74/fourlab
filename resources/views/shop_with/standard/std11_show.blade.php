...
<script type="text/javascript" charset="utf-8">
    ...
    const goList = () => location.href = "/shop/standard/std11/";

    const createAs = async () => {        
        if (validate() === false) return;
        try {
            const response = await axios({ url: `/shop/standard/std11/create`,
    ...
    const editAs = async () => {
        if (validate() === false) return;
        try {
            const response = await axios({ url: `/shop/standard/std11/edit`,
                method: 'post', data: $('form[name="f1"]').serialize()
            });
    ...
    const removeAs = async () => {
        ...
        try {
            const response = await axios({ url: `/shop/standard/std11/remove`,
