...
<script type="text/javascript" charset="utf-8">
	...
	const add = async() => {
		...
		const response = await axios({ url: `/shop/product/prd02/get-seq`, method: 'post',
			data: {
	...
	function save() {
		...
		axios({
			url: '/shop/product/prd02/save_product',
			method: 'post',
			data: {
				data: rows,
				sel_data: sel_rows,
	...
	function delete_img(prd_cd, seq) {
		...
		if (confirm("선택한 사진을 삭제하시겠습니까?")) {
			$.ajax({
				method: 'post',
				url: '/shop/product/prd02/del_img',
				data: {
					data_img: prd_cd,
					seq: seq
				},
	...
	function sel_seq() {
		...
		if (brand == "") {
			...
			$.ajax({
					method: 'post',
					url: '/shop/product/prd02/sel_seq',
					data: {
						prd_cd : prd_cd
					},
					success: function(data) {
	...
	function change_gender() {
		...
		$.ajax({
			method: 'post',
			url: '/shop/product/prd03/change-gender',
			data: {
				gender : gender
			},
	...
	function changeSelect() {
		...
		$.ajax({
			method: 'post',
			url: '/shop/product/prd02/change_seq',
			data: {
				prd_nm : prd_nm[1],
				prd_seq : prd_nm[0]
			},
		...
		const openBatchPopup = () => {
			const url = '/shop/product/prd02/batch';
