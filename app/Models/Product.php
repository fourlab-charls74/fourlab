    ...
    */
    ...
    /*
      ...
      Returns:
        $wonga - 원가
    */
    private function __GetPartnerGoodsWonga( $com_id, $price, $ord_date,$wonga = 0){
      ...
      return isset($row["wonga"])? $row["wonga"]:$wonga;
    }
    ...
    */
    public function Minus( $stock ){
      ...
      if( $goods_type == "S" || $goods_type == "I" ){ // 공급업체
        ...
        if($is_order){
          $sql = "
            select date_format(ord_date,'%Y%m%d') as ord_date, price,wonga
            from order_opt
            where ord_opt_no = '$ord_opt_no'
          ";
        ...
        // 주문 정보 있는 경우 해당 가격, 일자의 원가 얻기
        $ord_stock_wonga = ($is_order) ? $this->__GetPartnerGoodsWonga( $com_id, $ord_price, $ord_date,$ord_wonga ) : 0;
	...
	public function Sale($sale_type, $sale_rate)
	{
		...
		if( $sale_rate > 0 ){
			...
			$sql	= "
				...
				WHERE
				goods_no = :goods_no and ( sale_dt_yn = 'N' or sale_dt_yn is null )
			";
