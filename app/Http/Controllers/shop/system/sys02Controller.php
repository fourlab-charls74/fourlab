...
namespace App\Http\Controllers\shop\system;
...
class sys02Controller extends Controller
{
    ...
    public function create()
    {
        ...
        return view(Config::get('shop.shop.view') . '/system/sys02_show', $values);
    }
    ...
    public function show($code)
    {
        ...
        $sql =
            ...
            "
            select * from shop_controller a
			where entry = ( select entry from shop_controller where menu_no = :entry ) and is_del = 0 and state >= 0
			order by seq
            ";
    ...
    public function search(Request $req)
    {
        ...
        while($row = $stmt->fetch(PDO::FETCH_ASSOC))
        {
            ...
            $sql = /** @lang text */
                "
                select a.*, ( select count(*) from shop_controller where entry = a.menu_no ) as cnt
                from shop_controller a
    ...
    public function store(Request $request)
    {
        ...
        $user_cnt = DB::table('shop_controller')
            ->where('pid', $pid)->count();
        if ($user_cnt == 0) {
            ...
            try {
                DB::transaction(function () use (&$result, $shop_controller) {
                    DB::table('shop_controller')->insert($shop_controller);
                });
                $code = 200;
                $msg = "";
            } catch (Exception $e) {
    ...
    public function update($code, Request $request)
    {
        ...
        try {
            DB::transaction(function () use (&$result, $code, $shop_controller, $roles) {
                DB::table('shop_controller')
                    ->where('menu_no', '=', $code)
                    ->update($shop_controller);
    ...
    public function delete($code, Request $req)
    {
        try {
            DB::transaction(function () use (&$result, $code) {
                DB::table('shop_controller')->where('menu_no', $code)->delete();
            });
