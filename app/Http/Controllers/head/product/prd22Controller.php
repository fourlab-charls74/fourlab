...
class prd22Controller extends Controller
{
    public function index()
    {
        $code  = "criteo_shopping";
        $edate = date("Y-m-d");
        ...
        return view(Config::get('shop.head.view') . '/product/prd22', $values);
    }
    ...
    public function show($code)
    {
        ...
        $goods_all_url 	= sprintf("https://%s/api/criteo/criteo%s_all.xml", $cfg_domain, $_mall_id);
        ...
        $values = [
            ...
            'all_except_some' => $this->getAllExceptSome($shop_ad_goods_all), // 전체 상품 중 제외된 정보 가져오기
            'ads' => $this->getAds($shop_ad_goods_all->ad) // 광고 목록 출력
        ];

        return view(Config::get('shop.head.view') . '/product/prd22_show', $values);
    }

    /**
     * 전체 상품 연동 제외 - 대표 카테고리, 업체, 브랜드, 상품 조회
     */
    public function getAllExceptSome($info_row) {
    ...
    public function store($code, Request $request) 
    {
        ...
        try {
            DB::transaction(function () use (&$result, $code, $shop_ad_goods_all, $shop_ad_goods_set) {
