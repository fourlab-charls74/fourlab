...
class com02Controller extends Controller
{
	...
	public function Save(Request $request){
		...
		$thumb_img_url  = $request->input('thumb_img_url','');
		$thumb_img_nm   = $request->input('thumb_img_nm','');
			...
			if($b_no == "" && $type == "detail"){
				...
				if($thumb_img_url != '') {
					if( $board_config['functions'] & 16 ) {
						$contents['thumb_img_url'] = $thumb_img_url;
						$contents['thumb_img_nm'] = $thumb_img_nm;
						$board->uploadFile($b_no, $is_edit = 0, $contents);
					}
				}
			}else if ( $type == "detail" && $b_no !== "") {
				$contents = array(
					"board_id"		=> $board_id
				, "b_no"		=> $b_no
				, "subject"		=> $subject
	...
	public function image_save($no, $board_id = "", $img_name, $img_url){

		$base_path = "/data/board/" . $board_id;
		$img = [
			'file_nm' => "",
			'file_sz' => ""
		];
		...
		if(Storage::disk('public')->exists($base_path)) {
			if ($img_url != "") {
				$image      = preg_replace('/data:image\/(.*?);base64,/', '', $img_url);
				$save_file  = sprintf("%s/%s", $base_path, $img_name);

				Storage::disk('public')->put($save_file, base64_decode($image));
				$file = Storage::disk('public')->path($save_file);
				$img['file_nm'] = $img_name;
				$img['file_sz'] = round(filesize($file) / 1024);
			}
		}
		return $img;
	}
