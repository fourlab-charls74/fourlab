...
class sal03Controller extends Controller
{
    ...
    public function search(Request $request){
		...
		if($mobile_yn != "" || $app_yn != ""){
			$inner_join .=  " inner join order_mst om on o.ord_no = om.ord_no";
		}
	...
	public function search_chart(Request $request){
		...
		$sdate = str_replace("-","",$request->input('sdate',Carbon::now()->sub(12, 'month')->format('Ymd'))) . '01';
		$edate = str_replace("-","",$request->input('edate',date("Ymd"))). '31';;
		...
		$chart_sql = "
			select
				a.sale_date as month,
				qty_30 as qty,
				t.recv_amt_30 as recv_amt,
				ifnull((qty_30 + qty_60 + qty_61),0) as profit_qty,
				ifnull((t.recv_amt_30 + t.recv_amt_60 + t.recv_amt_61) + (t.point_amt_30 + t.point_amt_60 + t.point_amt_61 ) - (t.fee_amt_30 + t.fee_amt_60 + t.fee_amt_61 ),0) as swonga,
				ifnull((t.wonga_30 + t.wonga_60 + t.wonga_61), 0) as wonga,
				ifnull(((t.recv_amt_30 + t.recv_amt_60 + t.recv_amt_61) + (t.point_amt_30 + t.point_amt_60 + t.point_amt_61) - (t.fee_amt_30 + t.fee_amt_60 + t.fee_amt_61 ) - (t.wonga_30 + t.wonga_60 + t.wonga_61)), 0) as profit
			from (
				select date_format(d,'%Y%m') as sale_date from mdate
				where d >='$sdate' and d <= '$edate'
