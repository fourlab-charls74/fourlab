...
class cs73Controller extends Controller
{
    // 월별 클레임 통계 컨트롤러
    public function index() {
        ...
        return view( Config::get('shop.head.view') . '/cs/cs73',$values);
    }
    ...
    public function search(Request $request){
        ...
        $sql = /** @lang text */
            "
			select 
				goods_no,goods_sub,style_no,goods_nm,
                ifnull(sum(if(clm_reason in($claim_no_str) ,cnt,0)),0) as clm_cnt
				$claim_menu
			from (
					select 
						g.goods_no,g.goods_sub,g.style_no,g.goods_nm,c.clm_reason,count(*) as cnt
					...
					where req_date >= '$sdate' and req_date <= date_add('$edate',interval 1 day)	 
						$where
					group by g.goods_no,g.goods_sub,goods_nm,g.style_no,clm_reason
				) c 
			group by goods_no,goods_sub,style_no,goods_nm  
