...
class cs71Controller extends Controller
{
    // 일별 클레임 통계 컨트롤러
    public function index() {
        ...
        return view( Config::get('shop.partner.view') . '/cs/cs71',$values);
    }
    ...
    public function search(Request $request){
        ...
        /** @lang text */
        $sql = "
			      select date_format(a.d,'%Y%m%d') as reg_date
                ...
                select d 
                  from mdate 
                  where d >='$sdate' 
                  and d <='$edate' order by d desc
              ) a 
              left outer join (
                select
                  date_format(c.req_date,'%Y%m%d') as req_date,c.clm_reason,count(*) as cnt
                ...
                where req_date >= '$sdate' and req_date <= date_add('$edate',interval 1 day)
                  $where
                group by date_format(req_date,'%Y%m%d'),clm_reason
              ) c 
              on a.d = c.req_date
            group by a.d
            order by a.d desc
    ...
    public function popup(Request $request) {
        ...
        return view( Config::get('shop.partner.view') . '/cs/cs71_show',$values);
