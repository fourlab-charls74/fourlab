...
//        }
	...
	*/
	...
	public function get_addinfo($goods_no){
     ...
     // 입점업체 상품코드를 수정
     public function mall_prd_cd_update(Request $request) {

        $goods_no    = $request->input('goods_no');
        $com_id      = $request->input('com_id');
        $mall_prd_cd = $request->input('mall_prd_cd', null);
        ...
        try {
            ...
            ->where('goods_no', $goods_no)
            ->update([
                'mall_prd_cd' => $mall_prd_cd
            ]);
        } catch (\Exception $exception) {
            if ($exception->getCode() == 23000) {
                return response()->json([
                    'success' => false,
                    'message' => '기존 사용중인 입점업체 상품코드입니다.'
                ], 200);
            }
        }
        return response()->json([
            'success' => true,
            'message' => '입점업체 상품코드를 등록했습니다.'
        ], 200);
    }
