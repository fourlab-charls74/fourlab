...
class Head
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if($is_loin === false && ($path != "/head/login" && $path != "/head/signUp" && strpos($path, "head/api/sabangnet")  === false) && Auth::guard('head')->check() == false){
            return redirect('/head/login');
        } else {

			if( $path != "/head/login" && $path != "/head/signUp" && strpos($path, "head/api/sabangnet")  === false )
			{
